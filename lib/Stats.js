...
Stats.prototype.toJson = function toJson(options, forToString) {
	...
	function fnModule(module) {
		var obj = {
			...
			inlineMode: module.inlineMode,
			inlinedInto: module.inlinedInto && module.inlinedInto.identifier(),
			inlinedIntoId: module.inlinedInto && module.inlinedInto.id,
...
Stats.jsonToString = function jsonToString(obj, useColors) {
	...
	function processModuleAttributes(module) {
		...
		if(module.inlineMode === "hoist") {
			colors.magenta(" [inlined into [" + module.inlinedIntoId + "]]");
		} else if(module.inlineMode === "inline") {
			colors.magenta(" [half-inlined into [" + module.inlinedIntoId + "]]");
		}
		if(!module.cacheable) {
