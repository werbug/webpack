...
DependenciesBlock.prototype.isApiRequired = function(name) {
	return this.dependencies.some(function(dep) {
		var requiredApi = dep.requiredApi;
		if(typeof requiredApi === "function")
			requiredApi = dep.requiredApi();
		return requiredApi.indexOf(name) >= 0;
	}) || this.blocks.concat(this.variables).some(function(item) {
		return item.isApiRequired(name);
	});
};
