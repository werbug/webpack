...
Template.toIdentifier = function(str, middle) {
	if(typeof str === "number") str = "" + str;
	if(typeof str !== "string") return "";
	if(!middle)
		str = str.replace(/^[^a-zA-Z$_]/, "_");
...
Template.prototype.getModuleToRender = function(modules) {
	return modules.filter(function(module) {
		return !module.inlineMode;
	})
}

Template.prototype.getModulesArrayBounds = function(modules) {
	module = this.getModuleToRender(modules);
...
Template.prototype.renderChunkModules = function(chunk, moduleTemplate, dependencyTemplates, prefix) {
	...
	var renderedModules = this.getModuleToRender(chunk.modules);
	if(renderedModules.length === 0) {
		source.add("[]");
		return source;
	}
	var bounds = this.getModulesArrayBounds(renderedModules);
	if(bounds) {
		...
		// Render an object
		source.add("{\n");
		renderedModules.slice().sort(function(a, b) {
			var aId = a.id + "";
			var bId = b.id + "";
			if(aId < bId) return -1;
			if(aId > bId) return 1;
